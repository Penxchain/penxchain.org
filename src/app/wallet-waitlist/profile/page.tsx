...
import { getLevelInfo, socialTasks, dailyTasks } from '../lib/waitlist-data';
...
export default function ProfilePage() {
  ...
  const completedSocialTasksCount = user.completedTasks.filter((taskId) =>
    socialTasks.some((t) => t.id === taskId)
  ).length;

  const completedDailyTasksCount = user.completedTasks.filter((taskId) =>
    dailyTasks.some((t) => t.id === taskId)
  ).length;
               ...
               <div className="flex-1 w-full">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                     <div>
                        ...
                        <h1 className="text-4xl font-bold text-white mb-1 tracking-tight">{user.username}</h1>
                        <div className="flex items-center gap-2 text-white/50 text-sm">
                           <Shield className="w-3 h-3" />
                           <span className="font-mono">{user.id.substring(0, 8)}...{user.id.substring(user.id.length - 4)}</span>
                        </div>
                     </div>
                     <div className="text-right">
                        <div className="text-[10px] text-white/40 font-mono uppercase tracking-widest mb-1">Current Standing</div>
                        <div className="text-2xl font-bold text-white flex items-center justify-end gap-2">
                           <span className="text-[#2547D0]">Lvl {levelInfo.level}</span>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {[
                      { label: 'PXP Earned', value: user.points.toLocaleString(), color: '#2547D0' },
                      { label: 'Global Rank', value: `#${user.rank}`, color: '#ffffff' },
                      { label: 'Referrals', value: user.referralCount, color: '#00a3ff' },
                      { label: 'Reputation', value: 'High', color: '#0ce50c' },
                    ].map((stat, idx) => (
                      <div key={idx} className="p-3 bg-black/20 border border-white/5 rounded-lg backdrop-blur-sm">
  ...
  <div className="flex items-end justify-between mb-4">
    <div className="space-y-1">
      ...
      <div className="flex items-baseline gap-2">
        ...
        <span className="text-xs text-[#2547D0] font-mono font-bold">% SYNCED</span>
      </div>
    </div>
    ...
    <div className="text-right">
      ...
      <div className="flex items-center gap-2 justify-end">
        <span className="text-sm font-bold text-white font-mono">
          {levelInfo.nextLevelPoints ? (levelInfo.nextLevelPoints - user.points).toLocaleString() : 'MAXED'}
        </span>
        <span className="text-[10px] text-[#0ce50c] font-bold tracking-tighter">PXP REQ</span>
      </div>
    </div>
  </div>
      ...
      SYNCING SYSTEM DATA...
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          >
            ...
            <div className="space-y-4">
              {[
                ...
                { icon: Calendar, label: 'Initialization Date', value: joinDate },
                { icon: Award, label: 'Referral Code', value: user.referralCode, mono: true, copy: true },
              ].map((item, idx) => (
                <div key={idx} className="flex items-start gap-3 p-3 rounded-lg hover:bg-white/[0.02] transition-colors">
                  <div className="mt-1 p-1.5 bg-white/5 rounded-md">
                     <item.icon className="w-3.5 h-3.5 text-white/70" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] text-white/40 font-mono uppercase tracking-wider mb-0.5">{item.label}</p>
                    <p className={`text-white font-medium text-sm truncate ${item.mono ? 'font-mono text-[#2547D0]' : ''}`}>
                      {item.value}
                    </p>
                  </div>
                </div>
          ...
          >
            ...
            <div className="space-y-4">
              {[
                { icon: CheckCircle2, label: 'Total Completed', value: user.completedTasks.length.toString(), color: '#0ce50c' },
                { icon: Users, label: 'Social Protocols', value: `${completedSocialTasksCount}/${socialTasks.length}`, color: '#2547D0' },
                { icon: Clock, label: 'Daily Streaks', value: completedDailyTasksCount.toString(), color: '#00a3ff' },
              ].map((item, idx) => (
        ...
        <motion.div
         ...
         >
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
             {[
               { title: 'Early Adopter', desc: 'Joined the genesis waitlist', icon: Award, color: '#2547D0', achieved: true },
               { title: 'Task Master', desc: 'Completed 5+ protocols', icon: CheckCircle2, color: '#0ce50c', achieved: user.completedTasks.length >= 5 },
               { title: 'Influencer', desc: 'Referred 3+ users', icon: Users, color: '#00a3ff', achieved: user.referralCount >= 3 }
             ].map((badge, idx) => (
               <div 
                  key={idx} 
               ...
               >
                 <div className="w-10 h-10 rounded-lg flex items-center justify-center mb-4" style={{
                    backgroundColor: badge.achieved ? `${badge.color}15` : 'rgba(255,255,255,0.05)',
                    color: badge.achieved ? badge.color : 'white'
                 }}>
                   <badge.icon className="w-5 h-5" />
                 </div>
                 <h3 className={`font-bold text-sm mb-1 ${badge.achieved ? 'text-white' : 'text-white/50'}`}>{badge.title}</h3>
                 <p className="text-[10px] text-white/40 uppercase tracking-wide">{badge.desc}</p>
               </div>
             ))}
           </div>
         </motion.div>
